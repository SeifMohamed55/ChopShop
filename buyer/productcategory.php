<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/homeStyleSheet.css">
    <script src="Project.js"></script>
    <title>Home</title>

</head>

<body>
    <div>
        <div>
            <header>
                <div id="nav">
                    <ul>
                        <li><a href="home.html" class="navitem" id="home">Home</a></li>
                        <li><a href="Categories.html" class="navitem">Categories</a></li>
                        <li><a href="MyCart.html" class="navitem">My cart</a></li>
                        <li><a href="MyProfile.html" class="navitem">Profile</a></li>
                        <li style="float: right;"><a href="../index.php" class="navitem" id="z">Log out</a></li>
                    </ul>
                </div>
            </header>
            <p id="sellers">our top sellers :</p>
            <div id="photos">
                <div class="gallery">
                    <a href="product.html"><img src="" id="WM1"></a>
                    <div class="desc">Waffle Maker WX2850</div>
                </div>
                <div class="gallery">
                    <a href="product.html"><img src="" id="VC1">

                        <div class="desc">Vegetable container</div>
                </div>
                <div class="gallery">
                    <a href="product.html"><img src="" id="ASR1"></a>
                    <div class="desc">Avocado seed remover</div>
                </div>
                <div class="gallery">
                    <a href="product.html"><img src="" id="WC1"></a>
                    <div class="desc">Water colors</div>
                </div>
                <div class="gallery">
                    <a href="product.html"><img src="" id="S1"></a>
                    <div class="desc">Shoes</div>
                </div>
                <div class="gallery">
                    <a href="product.html"><img src="" id="TR1"></a>
                    <div class="desc">Toothpaste remover</div>
                </div>
        </div>
        <footer>
            <div id="back">
                <a href="#home">Back Top</a>
            </div>
            <div id="about">
                <ul>
                    <li id="Y"><a href="https://www.youtube.com/@GoogleArabia" target="_blank">Youtube</a></li>
                    <li id="F"><a href="https://www.facebook.com/Google/" target="_blank">Facebook</a></li>
                    <li id="T"><a href="https://twitter.com/Google" target="_blank">Twitter</a></li>
                    <li id="I"><a href="https://www.instagram.com/google/" target="_blank">Instagram</a></li>
                </ul>
            </div>
        </footer>
    </div>
</body>
<script src="for_abdo_only.js"></script>
</html>